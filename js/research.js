...
(function () {
  ...
  function ensureIndex() {
    ...
    msIndex = new window.MiniSearch({
      idField: 'id',
      fields: ['name', 'school', 'bio', 'summary', 'keywords'],
      storeFields: ['id'],
    });
  ...
  function searchNodes(query, wikiContext) {
    ...
    if (indexed && msIndex) {
      ...
      msIndex.search(query, {
        boost:  { name: 4, keywords: 3, summary: 2, school: 2, bio: 1 },
        fuzzy:  0.2,
        prefix: true,
      }).forEach(function (r) {
      ...
      if (wikiContext) {
        ...
        msIndex.search(enriched, {
          boost:  { keywords: 3, summary: 2, bio: 2, school: 1 },
          fuzzy:  0.15,
          prefix: true,
        }).forEach(function (r) {
  ...
  function openPanel() {
    ...
    // Position panel above current FAB location (desktop only)
    if (window.innerWidth > 768) {
      var fabRight  = parseFloat(fab.style.right)  || 24;
      var fabBottom = parseFloat(fab.style.bottom) || 24;
      panel.style.right  = fabRight  + 'px';
      panel.style.bottom = (fabBottom + 60) + 'px';
    }
  }
  ...
  function initDraggableFab() {
    ...
    function applyFabPos(right, bottom) {
      right  = clampVal(right,  4, window.innerWidth  - FAB_SIZE - 4);
      bottom = clampVal(bottom, 4, window.innerHeight - FAB_SIZE - 4);
      ...
      if (window.innerWidth > 768) {
        ...
        if (panel) {
          ...
          panel.style.bottom = (bottom + FAB_SIZE + 8) + 'px';
        }
        if (tooltip) {
          tooltip.style.right  = right + 'px';
          tooltip.style.bottom = (bottom + FAB_SIZE + 8) + 'px';
        }
      }
    }

    function saveFabPos() {
      var right  = parseFloat(fab.style.right)  || 24;
      var bottom = parseFloat(fab.style.bottom) || 24;
      try { localStorage.setItem('research-fab-pos', JSON.stringify({ right: right, bottom: bottom })); } catch (_) {}
    }

    // Load persisted position
    try {
      var saved = JSON.parse(localStorage.getItem('research-fab-pos') || 'null');
      if (saved && typeof saved.right === 'number' && typeof saved.bottom === 'number') {
        applyFabPos(saved.right, saved.bottom);
      }
    } catch (_) {}

    // Re-clamp when window is resized (e.g. rotation, zoom)
    window.addEventListener('resize', function () {
      applyFabPos(parseFloat(fab.style.right) || 24, parseFloat(fab.style.bottom) || 24);
    });
    ...
    fab.addEventListener('mousedown', function (e) {
      ...
      startX       = e.clientX;
      startY       = e.clientY;
      startRight   = parseFloat(fab.style.right)  || 24;
      startBottom  = parseFloat(fab.style.bottom) || 24;
    ...
    document.addEventListener('mousemove', function (e) {
      ...
      if (didDrag) applyFabPos(startRight - dx, startBottom + dy);
    });
    ...
    document.addEventListener('mouseup', function () {
      ...
      if (didDrag) {
        saveFabPos();
        // Prevent the next click from opening the panel
        fab.addEventListener('click', function blocker(e) {
    ...
    document.addEventListener('touchmove', function (e) {
      ...
      if (Math.abs(dx) > 4 || Math.abs(dy) > 4) didDrag = true;
      if (didDrag) {
        applyFabPos(startRight - dx, startBottom + dy);
        e.preventDefault();
      }
    }, { passive: false });
  ...
  window.addEventListener('load', function () {
    ...
    // Set up draggable FAB (must run before initTooltip so positions are correct)
    initDraggableFab();
