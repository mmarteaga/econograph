...
(function () {
  ...
  function makeRow(n) {
    ...
    // Row extra: keyword pills if available, else first-sentence bio preview
    let rowExtraHtml = '';
    if (n.keywords && n.keywords.length) {
      const kws = n.keywords.slice(0, 4);
      rowExtraHtml = '<div class="economist-keywords">' +
        kws.map(k => `<span class="keyword-pill">${escapeHtml(k)}</span>`).join('') +
        '</div>';
    } else if (n.bio && n.bio.length > 5) {
      const dot = n.bio.indexOf('.');
      const bioPreview = (dot > 10 && dot <= 130)
        ? n.bio.slice(0, dot + 1)
        : n.bio.slice(0, 110).trim() + (n.bio.length > 110 ? '…' : '');
      if (bioPreview) {
        rowExtraHtml = `<div class="economist-bio-preview">${escapeHtml(bioPreview)}</div>`;
      }
    }
  ...
  function openDetail(n) {
    ...
    // AI contributions section (synchronous — data is already in graph)
    renderContrib(n);
  ...
  /* ── 10. AI CONTRIBUTIONS SECTION ─────────────────────────────── */

  function renderContrib(n) {
    const section = document.getElementById('detail-contrib-section');
    if (!section) return;
    section.innerHTML = '';

    const hasSummary  = n.summary  && n.summary.length  > 0;
    const hasKeywords = n.keywords && n.keywords.length > 0;
    ...
    head.innerHTML = 'Key Contributions <span class="ai-badge" title="Generated by AI from Wikipedia">AI</span>';
    section.appendChild(head);

    if (hasSummary) {
      const text = document.createElement('p');
      text.className   = 'detail-contrib-text';
    ...
    if (hasKeywords) {
      ...
      pills.className = 'detail-keyword-pills';
      n.keywords.forEach(kw => {
        const pill       = document.createElement('span');
        pill.className   = 'detail-keyword-pill';
        pill.textContent = kw;
